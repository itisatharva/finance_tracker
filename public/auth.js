...
// Mobile browsers block popups — use redirect flow on mobile instead
const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);

// ── Handle Google redirect result on page load ───────────────────────────
// This fires when the page loads after returning from Google on mobile.
window.getRedirectResult(window.auth)
  .then(result => {
    if (!result || !result.user) return;  // no pending redirect, normal load
